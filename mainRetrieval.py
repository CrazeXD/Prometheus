...
# Orbital phase of the moon at planet mid-transit (in rad)
midTransitOrbphase = 0.65 * 2. * np.pi
...
# Wavelength grid specifications in cm: lower border, upper border, width of high-res. region (centered on the Na D line peaks),
...
# Spatial grid specifications: Midpoint of grid in x-direction (corresponding to the orbital radius of
spatialGrid = geom.Grid(W49b.a, 5. * W49b.R, 30,
                        W49b.hostStar.R, 40, 60, 0.25, 25)
...
# Add center-to-limb variations with parameters from Wyttenbach+2017
W49b.hostStar.addCLVparameters(0.34, 0.28)
# Could also add RM-effect here...


# Setting up to moon with an Io radius, and an unknown orbital radius / orbital phase
moon = bodies.Moon(midTransitOrbphase, R_moon *
                   const.R_Io, a_moon * W49b.R, W49b)

# Setting up an exosphere with a power law, with an exponent of -3.34 and a variable amount of Na atoms
moonExosphere = gasprop.TidallyHeatedMoon(3.34, moon)
# Define source rate as a function of moon orbital phase
moonExosphere.addSourceRateFunction(
    '/Users/agebek/Exoplanets/various/dishoom_sodium_Rio_X_0p3_June10_notides.txt', 241., 3.818e-23)
...
# In cm. This gives the width of the wavelength interval over which we average the transit depth signal. Default: 0.75 Angstrom.
filterBandwidth = 0.75e-8
...
for idx in range(len(orbphase)):
    ...
    SEL_filter = ((wavelength >= NaD2_center * shift[idx] - filterBandwidth / 2.) * (wavelength <= NaD2_center * shift[idx] + filterBandwidth / 2.)) + (
        (wavelength >= NaD1_center * shift[idx] - filterBandwidth / 2.) * (wavelength <= NaD1_center * shift[idx] + filterBandwidth / 2.))

    # Lightcurve, normalized
    lightcurve.append(np.mean(R[idx, SEL_filter]) / np.max(R[idx, :]))
